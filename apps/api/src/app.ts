...
import {
  createGameAccountService,
  type GameAccountService,
} from "./gameAccounts/gameAccountService.js";
...
function ensureConfigFiles(projectRoot: string) {
  const configsToCopy = [
    ...
    },
    {
      example: path.join(
        projectRoot,
        "apps/jx-services/mount/config/gateway/relay_config.ini.example",
      ),
          ...
          try {
            ...
            if (raw && raw.activeVersion && Array.isArray(raw.versions)) {
              ...
              if (activeVer && activeVer.path) {
                const envServerPath = activeVer.path.endsWith("/")
                  ? activeVer.path
                  : `${activeVer.path}/`;
  ...
  // Tự động tạo file version.json mặc định nếu chưa tồn tại
  const versionFilePath = path.join(projectRoot, "version.json");
  if (!existsSync(versionFilePath)) {
    try {
      writeFileSync(
        versionFilePath,
        JSON.stringify({ version: "v1.0.0", commit: "unknown" }, null, 2) + "\n",
        "utf8",
      );
    } catch {
      // Bỏ qua lỗi nếu không có quyền ghi
    }
  }
}
...
export async function buildApp(overrides: Partial<AppDeps> = {}) {
  ...
  const deps: AppDeps = {
    ...
    runCompose:
      ...
      ((args, options) =>
        runDockerCompose(args, config.projectRoot, options, {
          projectRoot: config.hostProjectRoot ?? config.projectRoot,
        })),
    runDocker:
      overrides.runDocker ?? ((args, options) => runDocker(args, config.projectRoot, options)),
    streamCompose:
      overrides.streamCompose ??
      ((args) =>
        runDockerComposeStream(args, config.projectRoot, {
          projectRoot: config.hostProjectRoot ?? config.projectRoot,
        })),
    gameAccounts:
      overrides.gameAccounts ??
      createGameAccountService(createMssqlGameAccountRepository(config.mssql)),
  };
